...
const languageHandlers = {
  ...
  latinCyrillic: {
    regex: /[\p{Script=Latin}\p{Script=Cyrillic}\p{Mark}\p{Punctuation}\p{Number}'\b]+/gu,
  ...
  return {
    ...
    configureServer(server) {
      server.middlewares.use(async (req, _, next) => {
        ...
        if (node_path.extname(toMarkdownFilePath) !== ".md")
          return next();
        const completeFilePath = node_path.join(_config.root, toMarkdownFilePath);
        if (!knownMarkdownFiles.has(toMarkdownFilePath.toLowerCase())) {
          try {
            const exists = await node_fs.existsSync(completeFilePath);
            if (!exists)
              return next();
            const stat = await node_fs.lstatSync(completeFilePath);
            if (!stat.isFile())
              return next();
        ...
        if (!knownMarkdownFiles.has(toMarkdownFilePath.toLowerCase()))
          return next();
        const content = await node_fs.readFileSync(completeFilePath, "utf-8");
    ...
    async handleHotUpdate(ctx) {
      ...
      if (node_path.extname(hotReloadingModuleFilePath) === ".md") {
